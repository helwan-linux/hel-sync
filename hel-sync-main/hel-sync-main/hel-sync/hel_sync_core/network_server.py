...
def ask():
    ...
    if permission_callback:
        ...
        # استدعاء الدالة التي تفتح QMessageBox في الواجهة
        return jsonify({"ok": permission_callback(count, format_size(int(size)))})
...
def handle_upload():
    ...
    for file in files:
        if file.filename:
            ...
            file.save(fpath)
            # إرسال إشارة للواجهة لتحديث القائمة وتنبيه المستخدم
            if gui_callback:
...
def clip():
    ...
    if text and clip_callback:
        # إرسال النص لصندوق النصوص الواردة في الواجهة
        clip_callback(text)
        return "OK"
    return "No text provided", 400

def start_network_service(gui_cb, clip_cb, token, perm_cb):
    global gui_callback, clip_callback, ACCESS_TOKEN, permission_callback
    gui_callback = gui_cb
    clip_callback = clip_cb
    ACCESS_TOKEN = token
    permission_callback = perm_cb
    # تشغيل السيرفر ليكون متاحاً لجميع الأجهزة على نفس الشبكة

...
def index():
    ...
    return render_template_string('''
        ...
        <head>
            ...
            <style>
                body { background: #0c0c0c; color: #eee; font-family: sans-serif; text-align: center; padding: 15px; transition: background 0.1s; }
                ...
                .buzz-active { background: #ff0000 !important; color: #fff !important; }
            </style>
        </head>
        <body id="mainBody">
            <h3>Hel-Sync Pro</h3>
            ...
            <div class="card">
                ...
                <button id="btnDlAll" onclick="downloadAll()" style="background:#28a745; margin-bottom:10px; display:none;">📥 DOWNLOAD ALL FILES</button>
                <div id="download_list"></div>
            </div>
            <div class="card">
                <h4>Text Clipboard</h4>
                <div id="pc_clip" style="background:#000; padding:10px; border:1px dashed #555; color:#0f0; margin-bottom:10px; font-size:13px; min-height: 20px;">Waiting...</div>
            ...
            <script>
                ...
                function downloadAll() {
                    const buttons = document.querySelectorAll('#download_list button');
                    buttons.forEach((btn, i) => { setTimeout(() => btn.click(), i * 500); });
                }
                ...
                function refreshFileList() {
                    fetch(`/get_files?token=${TOKEN}`).then(r => r.json()).then(files => {
                        ...
                        if (files.length === 0) {
                            list.innerHTML = '<p style="color:#666; font-size:12px;">No files shared yet.</p>';
                            ...
                            list.innerHTML = files.map((f, i) => `
                                <div class="file-item">
                                    ...
                                    <button onclick="forceDownload('/download/${i}?token=${TOKEN}')" style="width:auto; padding:5px 15px; background:#444; font-size:12px;">GET</button>
                                </div>
                            `).join('');
                        }
                    });
                ...
                function startMobileBuzz() {
                    ...
                    // 2. الصوت (Speech)
                    let msg = new SpeechSynthesisUtterance("Attention! PC is searching for this device.");
                    msg.lang = 'en-US';
                ...
                // التحديث اللحظي الشامل
                setInterval(() => {
                    fetch(`/get_clip?token=${TOKEN}`).then(r => r.json()).then(d => {
                        let display = document.getElementById('pc_clip');
                        if (d.text === "___BUZZ_NOW___") {
                            display.innerHTML = "<b style='color:white;'>🔔 PC IS SEARCHING FOR YOU!</b>";
                            startMobileBuzz();
                        } else {
                            display.innerText = d.text || "Waiting...";
                            stopMobileBuzz();
                        }
                    });
                    refreshFileList();
...
def remote_cmd():
    ...
    if action in ("volup", "voldown", "mute"):
        ...
        elif system == "Linux": os.system("loginctl lock-session || xdg-screensaver lock")
    elif action == "buzz":
        if system == "Windows" and winsound: winsound.Beep(1000, 400)
        elif system == "Linux": os.system("notify-send '🔔 Hel-Sync' 'Buzzing...' && paplay /usr/share/sounds/freedesktop/stereo/complete.oga || echo -e '\a'")

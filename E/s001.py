    ...
    return render_template_string('''
        ...
        <body>
            ...
            <div class="card">
                ...
                {% if f_list %}
                    <button onclick="downloadAll()" style="background:#28a745; margin-bottom:10px;">📥 DOWNLOAD ALL FILES</button>
                    <div id="download_list">
                        {% for f in f_list %}
                            <div class="file-item">
                                <span style="font-size: 13px;">📄 {{ f }}</span>
                                <button onclick="forceDownload('/download/{{ loop.index0 }}?token={{token}}')" style="width:auto; padding:5px 15px; background:#444; font-size:12px;">GET</button>
                            </div>
                        {% endfor %}
                    </div>
                {% else %}
            ...
            <script>
                ...
                function downloadAll() {
                    ...
                    let delay = 0;
                    buttons.forEach((btn) => {
                        // استخدام تأخير بسيط (500ms) لضمان أن المتصفح لا يحظر التحميلات المتعددة
                        setTimeout(() => {
                            btn.click();
                        }, delay);
                        delay += 500;
...
def download(file_id):
    ...
    if 0 <= file_id < len(FILES_TO_SHARE):
        ...
        # إجبار المتصفح على تحميل الملف بدلاً من فتحه (خاصة الملفات النصية)
        return send_from_directory(
            os.path.dirname(p), 
            os.path.basename(p), 
            as_attachment=True # هذا هو السطر السحري لحل مشكلة المتصفح
        )
    return "Not Found", 404

# بقية الدوال (remote_cmd, upload, get_clip, send_from_mobile, start_network_service) تبقى كما هي بدون تغيير
...
def remote_cmd():
    ...
    if action in ("volup", "voldown", "mute"):
        ...
        if system == "Windows":
            ...
            os.system("cinnamon-screensaver-command --lock || loginctl lock-session || dbus-send --type=method_call --dest=org.gnome.ScreenSaver /org/gnome/ScreenSaver org.gnome.ScreenSaver.Lock || qdbus org.freedesktop.ScreenSaver /ScreenSaver Lock || xdg-screensaver lock")
    elif action == "buzz":

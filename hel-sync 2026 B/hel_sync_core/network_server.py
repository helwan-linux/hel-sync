import os, logging, math, shutil, pyautogui
        ...
        <body>
            ...
            <div class="card">
                ...
                <div class="grid-ctrl">
                    ...
                    <button onclick="doCmd('lock')" style="background:#6c757d; grid-column: span 2;">🔒 Lock Windows</button>
                </div>
            </div>
            ...
            <script>
                function doCmd(action) {
                    fetch('/remote_cmd?token={{token}}&action=' + action);
                }
				...
				// 1. دالة التنبيه الشامل (صوت + اهتزاز + وميض)
				function playBuzzSound() {
					...
					msg.text = "I am here! Look at the light!"; 
					msg.lang = 'en-US';
					window.speechSynthesis.speak(msg);

					// وميض الشاشة (Flash Effect)
					flashScreen();
				}

				// 2. دالة وميض الشاشة (تغيير لون الخلفية بسرعة بين الأبيض والأحمر)
				function flashScreen() {
					let body = document.body;
					let count = 0;
					let interval = setInterval(() => {
						// إذا توقفت إشارة البحث، توقف عن الوميض
						if (document.getElementById('pc_clip').innerText.indexOf("SEARCHING") === -1) {
							clearInterval(interval);
							body.style.backgroundColor = "#0c0c0c"; // العودة للأسود
							return;
						}
						body.style.backgroundColor = (count % 2 === 0) ? "#ffffff" : "#ff0000";
						count++;
					}, 100); // سرعة الوميض (كل 0.1 ثانية)
				}

				// 3. المراقبة اللحظية (نفس الكود السابق مع إضافة الوميض)
				setInterval(() => {
					fetch('/get_clip?token={{token}}').then(r => r.json()).then(d => {
						...
						if (d.text === "___BUZZ_NOW___") {
							...
							document.body.style.backgroundColor = "#0c0c0c"; // التأكد من إطفاء الوميض
						}
					});
				}, 1000);
            </script>
...
def remote_cmd():
    ...
    if action == "mute": pyautogui.press('volumemute')
        ...
        if winsound: winsound.Beep(1000, 400); winsound.Beep(1200, 400)

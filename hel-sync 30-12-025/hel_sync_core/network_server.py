...
FILES_TO_SHARE = [] 
ACCESS_TOKEN = ""
OUTGOING_TEXT = "" # المتغير المسؤول عن عرض نصوص الكمبيوتر للموبايل
gui_callback = None
clip_callback = None
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .clipboard-pc { background:#222; padding:10px; border-radius:5px; border-left:4px solid #a349a4; color:#ddd; min-height:40px; white-space: pre-wrap; margin-bottom:10px; }
        </style>
    </head>
    <body>
        ...
        <div class="card">
            <h3>✂️ Shared Clipboard (From PC)</h3>
            <div class="clipboard-pc">{{out_txt}}</div>
            <button onclick="location.reload()" style="width:100%; background:#444; color:white; border:none; padding:8px; border-radius:5px;">Refresh to see updates</button>
        </div>

        <div class="card">
            <h3>✂️ Send Text to PC</h3>
    ...
    ''', token=token, out_txt=OUTGOING_TEXT)
